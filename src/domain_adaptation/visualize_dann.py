"""
Comprehensive LOSO visualisation: all 6 methods (baselines + CORAL + MMD + DANN).

Generates in results/domain_adaptation/:
  1. loso_comparison_full.png
       Bar chart — all 6 methods, error bars, per-fold scatter overlay.

  2. loso_da_delta_full.png
       Per-fold delta line chart — CORAL, MMD, DANN vs MLP baseline,
       showing which subjects benefit from each method.

Usage:
    python src/domain_adaptation/visualize_dann.py
    python src/domain_adaptation/visualize_dann.py --show
"""
...
def plot_all_methods(save=True, show=False):
    ...
    entries = [
        ...
        ("MLP+\nMMD",         "loso_mlp_mmd_results.npy",   RESULTS_DA_DIR, "#CCB974"),
        ("MLP+\nDANN (ours)", "loso_mlp_dann_results.npy",  RESULTS_DA_DIR, "#E377C2"),
    ]
    ...
    # Bracket over the three DA methods
    da_left  = x[3] - 0.35
    da_right = x[5] + 0.35
    bracket_y = max(means[3:6]) + max(stds[3:6]) + 5
    ax.annotate("", xy=(da_right, bracket_y), xytext=(da_left, bracket_y),
                arrowprops=dict(arrowstyle="-", color="#555", lw=1.5))
    ax.text((da_left + da_right) / 2, bracket_y + 0.5,
            "Domain Adaptation Methods", ha="center",
    ...
    ax.set_title("Cross-Subject (LOSO) Emotion Recognition — SEED Dataset\n"
                 "Baselines vs Domain Adaptation (CORAL / MMD / DANN)",
    ...
    if save:
        path = os.path.join(RESULTS_DA_DIR, "loso_comparison_full.png")
...
def plot_fold_delta(save=True, show=False):
    ...
    da_methods = [
        (coral, "#8172B2", "o", "MLP+CORAL − MLP"),
        (mmd,   "#CCB974", "s", "MLP+MMD − MLP"),
        (dann,  "#E377C2", "^", "MLP+DANN − MLP"),
    ]

    for r, color, marker, label in da_methods:
        if r is None:
            continue
        delta = (r["accs"] - mlp["accs"]) * 100
        ax.plot(folds, delta, marker=marker, color=color,
                linewidth=1.8, markersize=6, label=label)
        ax.fill_between(folds, delta, 0,
                        where=(delta >= 0), alpha=0.12, color=color)
        ax.fill_between(folds, delta, 0,
    ...
    ax.set_title("Per-fold Improvement of Each DA Method over MLP Baseline",
    ...
    if save:
        path = os.path.join(RESULTS_DA_DIR, "loso_da_delta_full.png")
        fig.savefig(path, dpi=150)
        print(f"Saved → {path}")
    if show:
        plt.show()
