...
Based on src/domain_adaptation/train_dann.py. Only changes:
...
Usage:
    python testsrc/domain_adaptation/train_dann.py
    python testsrc/domain_adaptation/train_dann.py --lambda_dann 0.5
    python testsrc/domain_adaptation/train_dann.py --quiet
"""
...
def run_dann_experiment(lambda_dann: float = 1.0, verbose: bool = True) -> dict:
    ...
    # ── changed: save as JSON ─────────────────────────────────────────────────
    _save_json(result, "loso_mlp_dann_results.json")

    # ── changed: save epoch log as txt ─────────────────────────────────────────
    log_path = os.path.join(RESULTS_DIR, "loso_mlp_dann_epoch_log.txt")
