...
Based on src/domain_adaptation/train_mmd.py. Only changes:
...
Usage:
    python testsrc/domain_adaptation/train_mmd.py
    python testsrc/domain_adaptation/train_mmd.py --lambda_mmd 0.5
    python testsrc/domain_adaptation/train_mmd.py --quiet
"""
...
def run_mmd_experiment(lambda_mmd: float = 1.0, verbose: bool = True) -> dict:
    ...
    # ── changed: save as JSON ─────────────────────────────────────────────────
    _save_json(result, "loso_mlp_mmd_results.json")

    # ── changed: save epoch log as txt ─────────────────────────────────────────
    log_path = os.path.join(RESULTS_DIR, "loso_mlp_mmd_epoch_log.txt")
