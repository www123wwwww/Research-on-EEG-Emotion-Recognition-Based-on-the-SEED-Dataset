...
Based on src/domain_adaptation/train_mmd.py. Only change:
  - Per-subject z-score normalization BEFORE merging into source domain.
    ...
    domain, making subsequent MMD alignment between source and target
    more meaningful.

Results saved to testresults/ (as JSON + epoch log txt).

Usage:
    python testsrc/domain_adaptation/train2_mmd.py
    python testsrc/domain_adaptation/train2_mmd.py --lambda_mmd 0.5
    python testsrc/domain_adaptation/train2_mmd.py --quiet
"""
...
def run_mmd_experiment(lambda_mmd: float = 1.0, verbose: bool = True) -> dict:
    ...
    for i, split in enumerate(splits):
        ...
        acc, fold_logs = run_mmd_fold(split, lambda_mmd=lambda_mmd)
        accs.append(acc)
    ...
    std_acc  = np.std(accs)
    print(f"\n>>> LOSO | MLP+MMD + per-subject norm (λ={lambda_mmd}): "
          f"acc = {mean_acc*100:.2f}% ± {std_acc*100:.2f}%")

    result = {
        "model":      "mlp_mmd_subject_norm",
        "split":      "loso",
    ...
    # ── changed: save as JSON ─────────────────────────────────────────────────
    _save_json(result, "loso_mlp_mmd_subject_norm_results.json")

    # ── changed: save epoch log as txt ─────────────────────────────────────────
    log_path = os.path.join(RESULTS_DIR, "loso_mlp_mmd_subject_norm_epoch_log.txt")
...
def main():
    parser = argparse.ArgumentParser(
        description="Train MLP + MMD with per-subject normalization on SEED (LOSO split)."
    )
    parser.add_argument(
    ...
    print(f"Model      : MLP + MMD + per-subject norm  |  Split: LOSO")
