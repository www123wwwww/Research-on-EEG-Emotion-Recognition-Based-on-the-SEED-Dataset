...
Unified hyperparameters (no early stopping):
  ...
  BATCH_SIZE   = 128
  WEIGHT_DECAY = 1e-4
  VAL_RATIO    = 0.2   (temporal split: first 80% train, last 20% validation)
  SEED         = 42
...
Results saved to: ./results/ (relative to script directory)
...
def run_baseline_fold(split, norm_mode):
    ...
    X_tr, y_tr, X_val, y_val = _temporal_split(X_tr_all, y_tr_all)
...
def run_weighted_mmd_fold(split_w, lambda_mmd, temperature):
    ...
    for s, (X_s_raw, y_s) in source_data.items():
        ...
        X_tr_s, y_tr_s, X_val_s, y_val_s = _temporal_split(X_s, y_s)
