...
每位被试每个session独立训练和测试：使用该被试该session的9个训练trial（499个样本）的前80%训练、后20%验证（时序划分，不打乱顺序），6个测试trial（343个样本）评估。共12位被试×3个session=36次实验，取平均准确率。

此设置衡量的是**在不涉及跨被试迁移**的情况下的分类能力，作为跨被试识别性能的参考基线。注意，由于每session训练数据仅约401个样本，该基线受限于数据量，不代表被试内情绪识别的理论上限。

#### 3.2.2 B组：跨被试基线（LOSO）

采用留一被试交叉验证（LOSO）协议：每次将1位被试的全部2526个样本作为测试集（目标域），其余11位被试的数据（27,786个样本）作为训练集（源域）。从源域中按时序取前80%训练、后20%验证（时序划分，不打乱顺序），用于模型选择。
...
对于A组实验，训练/验证/测试划分在每次session内完成（前80%训练/后20%验证，时序划分）。对于B/C/D/F组，LOSO的验证集从源域训练数据中按时间顺序取前80%训练、后20%验证（时序划分，不打乱顺序）。
...
1. **被试内识别准确率为66.93%**，显著高于三分类随机水平（33.3%），但考虑到模型仅需区分三类情绪且已见过该被试数据，这一准确率并不算高。这反映了EEG情绪识别本身的内在难度：即使是同一被试，不同session间的情绪脑电模式也存在显著波动（如被试9从78.13%降至51.90%）。

2. **被试间差异极大**：被试3达到79.30%（且session 3达到100%），而被试2仅45.68%（最低session低至31.78%，接近随机水平）。个体间的这种巨大差异——无论是整体准确率水平还是session间稳定性——构成了跨被试迁移的主要挑战。

3. **同一被试不同session间波动较大**：如被试5从83.09%降至50.15%，被试9从78.13%降至51.90%。这种session间波动说明情绪状态在不同实验时段的稳定性较低，暗示(1)情绪脑电信号具有时变性；(2)SEED数据集的部分被试标注一致性可能存在问题；(3)跨被试域适应方法需要同时应对被试间差异和个体内的时变性。
...
1. **跨被试基线（55.20%）远低于被试内基线（66.93%）**，存在11.73%的域偏移差距。这11.73%的差距即为跨被试迁移需要弥补的主要性能损失。

2. **被试内归一化将LOSO基线提升至70.73%**，不仅弥补了域偏移差距（+15.54%），还超过了被试内识别水平（66.93%）。原因在于：被试内归一化消除了被试间的量级差异后，LOSO训练集（11位被试~27,786个样本）相比每被试session训练集（~401个样本）提供了更充足的训练数据，模型可以学到更具泛化性的特征。
...
3. **全局最优76.79%已超越被试内识别基线（66.93%）约10个百分点**。这一结果看似违反直觉——跨被试识别竟能超过被试内识别——但需注意两者是在不同条件下评估的：被试内实验每session仅有~401个训练样本，而LOSO训练集包含11位被试共~27,786个样本。在归一化消除了主要分布差异后，更大的训练数据量使模型能够学到更具判别性的特征表示。
...
2. **被试内归一化+MMD(λ=10.0)达到全局最优76.79%**，已超过被试内识别基线（66.93%）约10个百分点。需注意两者评估条件不同：被试内实验每session仅约401个训练样本，而LOSO训练集约27,786个样本。在归一化和域适应消除了主要分布差异后，更大的训练数据量使模型学到了更具判别性的特征表示。

3. **混合归一化+MMD(λ=1)的65.46%与被试内识别的66.93%几乎持平**，说明仅靠域适应即可将跨被试性能恢复到接近被试内水平，但受限于混合归一化未能消除被试间的量级差异，性能仍有上限。
...
被试内识别基线为66.93%，而跨被试混合归一化基线仅55.20%，存在约12个百分点的域偏移差距。这一差距反映了被试间分布差异（以均值和方差为主的量级差异）对模型泛化的严重影响。

被试内识别的准确率本身并不高（66.93%），部分被试（如被试2仅45.68%，最低session低至31.78%）的个体内识别接近随机水平。这暗示SEED数据集本身的标注质量或情绪诱发的稳定性存在一定局限。在被试2的情况下，即使是只使用该被试自身数据进行训练，模型的表现也极差，这远非域偏移所能解释，而可能与该被试的情绪主观报告与实际脑电活动的不一致性有关。

被试内归一化+MMD(λ=10.0)达到76.79%，超出被试内基线约10个百分点。但这一比较需考虑评估条件的差异——被试内实验每session仅约401个训练样本，而LOSO训练集包含约27,786个样本。在归一化和域适应消除了主要分布差异后，LOSO训练的大数据量优势使其能够学到更具判别性的特征表示，从而超越了只有少量训练数据的被试内基线。换言之，76.79% vs 66.93%的比较并不是公平的同条件对比，而应该理解为：在LOSO设定下，通过归一化+域适应消除域偏移后，模型的有效性能上限远高于被试内小数据量训练的基线。
...
5. **被试内识别结果的波动**：被试内识别的session间波动较大（如被试5从83.09%降至50.15%，被试9从78.13%降至51.90%），说明同一被试在不同时间的情绪脑电模式并不完全稳定。这一发现暗示：(1)情绪脑电信号本身具有时变性，即使在同一被试内也非平稳分布；(2)部分被试可能存在主观评分与实际脑电活动不一致的问题（如被试2仅45.68%，接近随机水平）；(3)跨被试的域适应方法需要同时应对被试间差异和个体内的时变性问题。
...
4. **全局最优76.79%超越了被试内识别基线（66.93%）**，但这一比较需考虑评估条件的差异：被试内实验仅使用约401个训练样本，而LOSO训练集约27,786个样本。因此76.79%并不代表跨被试方法"优于"被试内方法，而应理解为：在消除域偏移后，大数据量训练带来的性能提升远超域偏移造成的损失。
